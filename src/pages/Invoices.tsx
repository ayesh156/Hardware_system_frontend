      ...
      <div className={`p-3 rounded-lg border ${isDark ? 'bg-slate-800/50 border-slate-700/50' : 'bg-white border-slate-200 shadow-sm'}`}>
        <div className="flex items-center gap-2">
          <div className="relative flex-1 max-w-xs">
            ...
            <input type="text" placeholder="Search by invoice # or customer..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
          ...
          <button onClick={() => setSortOrder(sortOrder === 'asc' ? 'desc' : 'asc')} className={`p-1.5 rounded-lg border transition-colors ${isDark ? 'border-slate-700 hover:bg-slate-800' : 'border-slate-200 hover:bg-slate-50'}`}>
            {sortOrder === 'asc' ? <SortAsc className="w-3.5 h-3.5" /> : <SortDesc className="w-3.5 h-3.5" />}
          </button>
          <span className={`text-[10px] font-medium ml-auto ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{filteredInvoices.length} invoices</span>
        </div>
        {showFilters && (
          <div className={`flex flex-wrap gap-2 pt-3 mt-3 border-t ${isDark ? 'border-slate-700/50' : 'border-slate-200'}`}>
            <div className="flex-1 min-w-[160px]">
              ...
              ]} value={statusFilter} onChange={(v) => setStatusFilter(v)} placeholder="Search status..." isDark={isDark} />
            </div>
          </div>
        )}
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[900px]">
            <thead className={isDark ? 'bg-slate-800/80' : 'bg-slate-50'}>
              <tr>
                <th className={`px-2 py-2 text-[10px] font-semibold uppercase tracking-wider whitespace-nowrap text-left ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Invoice #</th>
                <th className={`px-2 py-2 text-[10px] font-semibold uppercase tracking-wider whitespace-nowrap text-left ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Customer</th>
                <th className={`px-2 py-2 text-[10px] font-semibold uppercase tracking-wider whitespace-nowrap text-left ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Issue Date</th>
                <th className={`px-2 py-2 text-[10px] font-semibold uppercase tracking-wider whitespace-nowrap text-left ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Due Date</th>
                <th className={`px-2 py-2 text-[10px] font-semibold uppercase tracking-wider whitespace-nowrap text-right ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Total</th>
                <th className={`px-2 py-2 text-[10px] font-semibold uppercase tracking-wider whitespace-nowrap text-center ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Status</th>
                <th className={`px-2 py-2 text-[10px] font-semibold uppercase tracking-wider whitespace-nowrap text-center ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Actions</th>
              </tr>
            </thead>
