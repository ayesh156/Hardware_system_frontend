...
export const ProductTable: React.FC<ProductTableProps> = ({ items, setItems }) => {
  ...
  const handleCellSave = useCallback((itemId: string, field: string, value: string | number) => {
    setItems((prev) => prev.map((i) => {
      ...
      if (field === 'storeQty') updated.status = deriveStatus(value as number);
      return updated;
    }));
    setCellEdit(null);
  }, [setItems]);

  const handleRowSave = useCallback((updated: InventoryProduct) => {
