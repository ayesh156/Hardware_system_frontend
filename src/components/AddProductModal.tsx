...
function sanitizeNumericInput(raw: string): string {
  // Allow empty string for graceful clearing
  if (raw === '' || raw === '.') return raw;
  // Strip leading zeros (e.g. "05" -> "5", "00" -> "0")
...
const ClearableInput: React.FC<ClearableInputProps> = ({ value, type = 'text', isNumeric, onChange, placeholder, hasError, isDark, className = '', min, step }) => {
  ...
  const handleFocus = () => {
    // If current value is exactly "0", clear it so user types fresh
...
export const AddProductModal: React.FC<AddProductModalProps> = ({ onSave, onClose }) => {
  const { theme } = useTheme();
  const isDark = theme === 'dark';
  const modalRef = useRef<HTMLDivElement>(null);

  // ── Clean canvas: all empty strings, all zeros ──
      ...
      <div ref={modalRef} onKeyDown={handleKeyDown} className={`relative w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-2xl border shadow-2xl animate-slide-in ${isDark ? 'bg-slate-800 border-slate-700/50' : 'bg-white border-slate-200'}`}>
        ...
        <div className={`sticky top-0 z-10 flex items-center justify-between px-5 py-3 border-b backdrop-blur-xl ${isDark ? 'bg-slate-800/90 border-slate-700/50' : 'bg-white/90 border-slate-200'}`}>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className={`text-sm font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>Add New Product</h2>
              <p className={`text-[10px] ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Fill in the details to create a new inventory item</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div>
              <FieldGroup label="Status" icon={<AlertTriangle className="w-3 h-3" />}>
                <input type="text" value={derivedStatus} disabled
            ...
            {/* Sales Type — searchable combobox */}
            <div>
              <FieldGroup label="Sales Type" icon={<ShoppingCart className="w-3 h-3" />}>
                <SearchCombobox options={SALES_TYPE_OPTIONS} value={form.salesType} onChange={(v) => updateField('salesType', v)} placeholder="Select type..." isDark={isDark} />
              </FieldGroup>
            </div>
        ...
        <div className={`sticky bottom-0 flex items-center justify-between px-5 py-3 border-t backdrop-blur-xl ${isDark ? 'bg-slate-800/90 border-slate-700/50' : 'bg-white/90 border-slate-200'}`}>
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={handleSubmit} className="flex items-center gap-1.5 px-4 py-2 text-xs font-semibold rounded-lg bg-gradient-to-r from-orange-500 to-rose-500 hover:from-orange-600 hover:to-rose-600 text-white shadow-lg shadow-orange-500/20 transition-all">
              <Save className="w-3.5 h-3.5" /> Add Product
            </button>
          </div>
        </div>
      </div>
